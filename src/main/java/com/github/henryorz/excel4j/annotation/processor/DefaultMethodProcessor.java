...
public class DefaultMethodProcessor implements MethodProcessor {
    ...
    public SheetConfig getSheetConfig() {
        ...
        Class<?>[] paramTypeParameters = new Class<?>[genericParameterTypes.length];
        for(int i = 0; i < genericParameterTypes.length; i++){
            Type item = genericParameterTypes[i];
                ...
                if (Collection.class.isAssignableFrom(rawType)) {
                    ...
                    if (actualTypeArguments != null && actualTypeArguments.length == 1) {
                        ...
                        if (paramTypeParameter instanceof Class<?>) {
                            ...
                            paramTypeParameters[i] = parameterizedParmType;
                        }
                    }
                }
            }
        }
        sheetConfig.setParameterizedParamTypes(paramTypeParameters);
